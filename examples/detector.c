...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        printf("%ld: %f, %f avg, %f rate, %lf seconds, %d images\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), what_time_is_it_now()-d_time, i*imgs);
        if(i%100==0){
#ifdef GPU
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        d_time=what_time_is_it_now();
        network_predict(net, X);
        printf("%s: Predicted in %f seconds.\n", input, what_time_is_it_now()-d_time);
        int nboxes = 0;
        ...
        if(outfile){
            ...
            time(&timep);
            p = localtime(&timep);
            sprintf(name, "%s%d.%02d.%02d %02d:%02d","results/", 1900+p->tm_year,1+p->tm_mon,p->tm_mday,p->tm_hour,p->tm_min);
            save_image(im, name);
#ifdef OPENCV
